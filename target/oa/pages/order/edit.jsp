...
<div class="weadmin-body">
    ...
    <form action="${pageContext.request.contextPath}/updbump.do" method="post" class="layui-form">
          <input type="hidden"  name="id"/>
        <div class="layui-form-item">
            <label class="layui-form-label">草拟人</label>
            <div class="layui-input-block">
                <input type="text" name="emp_name" lay-verify="title"  class="layui-input"  disabled >
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                 <c:if test="${user.dep_id==1}">
                     <input type="text" name="dep_name" lay-verify="title" disabled value="开发部" class="layui-input">
                 </c:if>
                <c:if test="${user.dep_id==2}">
                    <input type="text" name="dep_name" lay-verify="title" disabled value="销售部" class="layui-input">
                </c:if>
                <c:if test="${user.dep_id==3}">
                    <input type="text" name="dep_name" lay-verify="title" disabled value="人事部" class="layui-input">
                </c:if>
                <c:if test="${user.dep_id==4}">
                    <input type="text" name="dep_name" lay-verify="title" disabled value="财务部" class="layui-input">
                </c:if>
                <c:if test="${user.dep_id==5}">
                    <input type="text" name="dep_name" lay-verify="title" disabled value="行政部" class="layui-input">
                </c:if>
